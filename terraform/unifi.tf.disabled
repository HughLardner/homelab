...
resource "unifi_network" "vlan" {
  ...
  purpose = "vlan-only" # Must be one of corporate, guest, wan, or vlan-only.

  # IPv4 settings
  subnet       = "${each.value.networking.ipv4.subnet_prefix}.0/24"
  dhcp_start   = "${each.value.networking.ipv4.subnet_prefix}.200"
  dhcp_stop    = "${each.value.networking.ipv4.subnet_prefix}.55"
  dhcp_enabled = true
  igmp_snooping = false
  multicast_dns = false
